<?php
  ...
  $app->get('/marks',function() use($app){
    ...
    if(!isset($formation)) //If true, user has no formation
      $twigParameters = getTwigParameters('Voti Giornata '.$mid,$app['siteName'],'marks',$app['userMoney'],array('warning'=>'Non hai schierato la formazione, non ci sono voti Giornata  .$midda mostrare..','playedDays'=>$mid,'currentMid'=>$mid));
    elseif(!isset($marks)) //If true, user has no marks
      $twigParameters = getTwigParameters('Voti Giornata '.$mid,$app['siteName'],'marks',$app['userMoney'],array('playerMarks'=>array('POR'=>''),'playedDays'=>$mid,'currentMid'=>$mid));
    else //User has formation and marks
      $twigParameters = getTwigParameters('Voti Giornata '.$mid,$app['siteName'],'marks',$app['userMoney'],array('playerMarks'=>$playerMarks,'playedDays'=>$mid,'currentMid'=>$mid));
