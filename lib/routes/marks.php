<?php
  ...
  $app->get('/marks',function() use($app){
    ...
    */
    ...
    if(mysqli_affected_rows($app['conn']) === 0 ) //If 0, user has no formation
      $twigParameters = getTwigParameters('Voti',$app['siteName'],'marks',$app['userMoney'],array('warning'=>'Non hai schierato la formazione, non ci sono voti da mostrare..','playedDays'=>$mid,'currentMid'=>$mid));
    else
      $twigParameters = getTwigParameters('Voti',$app['siteName'],'marks',$app['userMoney'],array('playerMarks'=>$playerMarks,'playedDays'=>$mid,'currentMid'=>$mid));
  ...
  $app->get('/marks/{day}', function($day) use($app){
    ...
    /*
      ...
      didn't have any formation (and so we tell him there will be no marks to show).
    */
    $query = "SELECT * FROM user_formation WHERE MID = '$day' AND UID = '$uid'";
    $result = getResult($app['conn'],$query);
    if($result === false)
      $app->abort(452,__FILE__." (".__LINE__.")");
    
    if(!isset($formation)) //If 0, user has no formation
